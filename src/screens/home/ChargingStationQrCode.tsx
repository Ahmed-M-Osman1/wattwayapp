...
export default class ChargingStationQrCode extends BaseScreen<State, Props> {
  ...
  public async checkQrCodeDataAndNavigate(qrCodeData: string): Promise<void> {
    try {
      ...
      if (!endpointCloud) {
        Message.showError(t('qrCode.unknownEndpoint', { endpoint: chargingStationQrCode.endpoint }));
        return;
      }
      // Check Tenant
      ...
      if (chargingStationQrCode.tenantSubDomain !== this.currentTenant?.subdomain) {
        ...
        if (tenant) {
          ...
          this.setState(
            ...
            () => {
              Alert.alert(
                t('qrCode.wrongOrganizationTitle'),
                t('qrCode.wrongOrganizationMessage', { tenantName: tenant.name }),
                [
                  { text: t('general.no'), style: 'cancel', onPress: () => this.setState({ activateQrCode: true }) },
                  { text: t('general.yes'), onPress: async () => this.logoffAndNavigateToLogin(tenant) }
                ],
                { cancelable: false }
              );
            }
          ...
          this.setState(
            ...
            () => {
              Alert.alert(
                t('qrCode.unknownOrganizationTitle'),
                t('qrCode.unknownOrganizationMessage', { tenantName: chargingStationQrCode.tenantName }),
                [
                  { text: t('general.no'), style: 'cancel', onPress: () => this.setState({ activateQrCode: true }) },
                  { text: t('general.yes'), onPress: async () => this.saveTenantAndLogOff(chargingStationQrCode, endpointCloud) }
                ],
                { cancelable: false }
              );
            }
      ...
      try {
        ...
        if (!foundConnector) {
          Message.showError(t('qrCode.unknownConnector', { connectorID: chargingStationQrCode.connectorID }));
          return;
        }
      } catch (error) {
        // Charging Station not found
        Message.showError(t('qrCode.unknownChargingStation', { chargingStationID: chargingStationQrCode.chargingStationID }));
        return;
      }
    ...
    return (
      <View style={{backgroundColor: commonColor.containerBgColor}}>
        <HeaderComponent
          ...
          title={t('qrCode.scanChargingStationQrCodeTitle')}
          backArrow={true}
        />
       {activateQrCode && (
          <QRCodeScanner
